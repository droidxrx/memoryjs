...
export function STRUCTRON_TYPE_STRING(memoryjs: Library) {
	 ...
	 */
	return (handle: number, structAddress: number, platform: string, encoding?: BufferEncoding) => ({
		read(buffer: Buffer, offset: number) {
			...
			if (length > 15) {
				...
				return memoryjs.readMemory(handle, Number(pointer), 'string');
			}
		...
		write(value: string, context: any, offset: number) {
			...
			const length = memoryjs.readMemory(handle, lengthAddress, 'int');
			...
			if (length > 15 && value.length > 15) {
				...
				const pointer = memoryjs.readMemory(handle, structAddress + offset, 'pointer');
				memoryjs.writeMemory(handle, pointer, value, 'string');
